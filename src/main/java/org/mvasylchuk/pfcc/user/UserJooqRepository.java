...
public class UserJooqRepository {
    ...
    public ProfileDto getProfileByUserEmail(String email) {
        //TODO: Handle possible absence of user with provided email
        ProfileDto profileDto = ctx.select(USERS.EMAIL, USERS.PROFILE_CONFIGURED, USERS.CALORIES_AIM, USERS.CARBOHYDRATES_AIM, USERS.FAT_AIM, USERS.PROTEIN_AIM, USERS.PREFERRED_LANGUAGE)
                                   .from(USERS)
        ...
        List<DishDto> dishesList = ctx.select(DISH.asterisk())
                                      .from(DISH)
                                      .join(USERS)
                                      .on(DISH.OWNER_ID.eq(USERS.ID))
                                      .where(USERS.EMAIL.eq(email).and(DISH.DELETED.isFalse()))
                                      .fetch((dbdish) -> {
